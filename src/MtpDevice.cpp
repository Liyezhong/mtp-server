...
MtpDevice* MtpDevice::open(const char* deviceName, int fd) {
    ...
    if (!device) {
	    std::cout << "usb_device_new failed for " << deviceName << std::endl;
        return NULL;
    }
    ...
    while ((desc = usb_descriptor_iter_next(&iter)) != NULL) {
		...
		std::cout << "Found camera: \"" << manufacturerName << " \"" << productName << "\"" << std::endl;
                ...
                char* manufacturerName = usb_device_get_manufacturer_name(device);
                char* productName = usb_device_get_product_name(device);
		std::cout << "Found MTP device: \"" << manufacturerName << "\" \"" << productName << "\"" << std::endl;
                free(manufacturerName);
                free(productName);
            }
#if 0
			...
			std::cout << "endpoints not found" << std::endl;
		    ...
		    std::cout << "usb_device_claim_interface failed" << std::endl;
                usb_device_close(device);
                return NULL;
            }
...
std::cout << "*** FORMAT: " << MtpDebug::getFormatCodeName(format) << std::endl;
            ...
            if (props) {
                for (int j = 0; j < props->size(); j++) {
                    ...
                    if (property) {
                        ...
                        std::cout << "could not fetch property: "
                                   << MtpDebug::getObjectPropCodeName(prop) << std::endl;
                    }
                }
            }
        }
...
bool MtpDevice::readObject(MtpObjectHandle handle,
    ...
    if (sendRequest(MTP_OPERATION_GET_OBJECT)
		...
		std::cout << "readObject error objectSize: " << objectSize
        ...
        while (remaining > 0 || writeBuffer) {
            if (remaining > 0) {
                ...
                if (mData.readDataAsync(req)) {
                    std::cout << "readDataAsync failed" << std::endl;
                    goto fail;
                }
            } else {
                req = NULL;
...
bool MtpDevice::readObject(MtpObjectHandle handle, const char* destPath, int group, int perm) {
	std::cout << "readObject: " << destPath << std::endl;
    int fd = ::open(destPath, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    if (fd < 0) {
	std::cout << "open failed for " << destPath << std::endl;
        return false;
    }
...
bool MtpDevice::sendRequest(MtpOperationCode operation) {
	std::cout << "sendRequest: " << MtpDebug::getOperationCodeName(operation) << std::endl;
    mReceivedResponse = false;
...
std::cout << "readData returned " << ret << std::endl;
    if (ret >= MTP_CONTAINER_HEADER_SIZE) {
        if (mData.getContainerType() == MTP_CONTAINER_TYPE_RESPONSE) {
std::cout << "got response packet instead of data packet" << std::endl;
...
std::cout << "readResponse failed" << std::endl;
        return false;
    }
}
