...
std::cout << "MtpServer::run fd: " << fd << std::endl;

    while (1) {
        int ret = mRequest.read(fd);
        if (ret < 0) {
std::cout << "request read returned " << ret << std::endl;
            if (errno == ECANCELED) {
...
std::cout << "operation: " << MtpDebug::getOperationCodeName(operation) << std::endl;
        mRequest.dump();
        ...
        if (dataIn) {
            ...
            if (ret < 0) {
std::cout << "data read returned " << ret << std::endl;
                if (errno == ECANCELED) {
                    // return to top of loop and wait for next command
                    continue;
                }
...
std::cout << "sending data:" << std::endl;
                ...
                ret = mData.write(fd);
                if (ret < 0) {
std::cout << "request write returned " << ret << std::endl;
                    if (errno == ECANCELED) {
                        // return to top of loop and wait for next command
                        continue;
                    }
	    ...
	    std::cout << "sending response "
                    << std::hex << mResponse.getResponseCode() << std::dec << std::endl;
...
std::cout << "skipping response" << std::endl;
        }
    }
...
std::cout << "sendObjectPropChanged " << handle << " " << prop << std::endl;
    sendEvent(MTP_EVENT_OBJECT_PROP_CHANGED, handle, prop, 0);
}

void MtpServer::sendStoreAdded(MtpStorageID id) {
std::cout << "sendStoreAdded " << std::hex << id << std::dec << std::endl;
    sendEvent(MTP_EVENT_STORE_ADDED, id, 0, 0);
}

void MtpServer::sendStoreRemoved(MtpStorageID id) {
std::cout << "sendStoreRemoved " << std::hex << id << std::dec << std::endl;
...
std::cout << "mEvent.write returned " << ret << std::endl;
    }
}
...
std::cout << "ObjectEdit not found in removeEditObject" << std::endl;
}
...
std::cout << "expected SendObject after SendObjectInfo" << std::endl;
        mSendObjectHandle = kInvalidObjectHandle;
    }
    ...
    switch (operation) {
        ...
        case MTP_OPERATION_END_EDIT_OBJECT:
            ...
            break;
        default:
std::cout << "got unsupported command " << MtpDebug::getOperationCodeName(operation) << std::endl;
...
std::cout <<  __PRETTY_FUNCTION__ << std::endl;
    ...
    strcpy(prop_value, "arthur"); // manufacturer
    string.set(prop_value);
    mData.putString(string);   // Manufacturer

    strcpy(prop_value, "A00"); // model
    string.set(prop_value);
    mData.putString(string);   // Model
    string.set("1.0");
    mData.putString(string);   // Device Version

    strcpy(prop_value, "A0000613"); // ro.serialno
    string.set(prop_value);
...
MtpResponseCode MtpServer::doGetObjectPropValue() {
    ...
    std::cout << "GetObjectPropValue " << handle
            << " " << MtpDebug::getObjectPropCodeName(property) << std::endl;
...
MtpResponseCode MtpServer::doSetObjectPropValue() {
    ...
    MtpObjectHandle handle = mRequest.getParameter(1);
    MtpObjectProperty property = mRequest.getParameter(2);
    std::cout << "SetObjectPropValue " << handle
            << " " << MtpDebug::getObjectPropCodeName(property) << std::endl;
...
std::cout << "GetDevicePropValue " << MtpDebug::getDevicePropCodeName(property) << std::endl;
...
MtpResponseCode MtpServer::doSetDevicePropValue() {
    MtpDeviceProperty property = mRequest.getParameter(1);
std::cout << "SetDevicePropValue " << MtpDebug::getDevicePropCodeName(property) << std::endl;
...
MtpResponseCode MtpServer::doResetDevicePropValue() {
    MtpDeviceProperty property = mRequest.getParameter(1);
std::cout << "ResetDevicePropValue " << MtpDebug::getDevicePropCodeName(property) << std::endl;
...
MtpResponseCode MtpServer::doGetObjectPropList() {
    ...
    std::cout << "GetObjectPropList " << handle
...
std::cout << "MTP_SEND_FILE_WITH_HEADER returned " << ret << std::endl;
    close(mfr.fd);
    if (ret < 0) {
        if (errno == ECANCELED)
...
MtpResponseCode MtpServer::doSendObjectInfo() {
    ...
    std::cout << "name: " << (const char *) name
            << " format: " << std::hex << format << std::dec << std::endl;
    time_t modifiedTime;
    ...
    std::cout << "path: " << path.c_str() << " parent: " << parent
            << " storageID: " << std::hex << storageID << std::dec << std::endl;
...
std::cout << "Expected SendObjectInfo before SendObject" << std::endl;
...
std::cout << "receiving " << mSendObjectFilePath.c_str() << std::endl;
        // transfer the file
        ret = ioctl(mFD, MTP_RECEIVE_FILE, (unsigned long)&mfr);
std::cout << "MTP_RECEIVE_FILE returned " << ret << std::endl;
    }
    close(mfr.fd);
...
std::cout << "path too long: " << path << std::endl;
    }
    strcpy(pathbuf, path);
    ...
    if (!dir) {
std::cout << "opendir " << path << " failed" << std::endl;
        return;
    }
...
std::cout << "path " << path << "/" << name << " too long" << std::endl;
            continue;
        }
...
std::cout << "deletePath stat failed for " << path << std::endl;
    }
}
...
MtpResponseCode MtpServer::doDeleteObject() {
    ...
    if (result == MTP_RESPONSE_OK) {
std::cout << "deleting " << filePath.c_str() << std::endl;
...
MtpResponseCode MtpServer::doMoveObject() {
    ...
    if (result == MTP_RESPONSE_OK) {
std::cout << "moving " << filePath.c_str() << " to " << newPath.c_str() << std::endl;
...
MtpResponseCode MtpServer::doGetObjectPropDesc() {
    ...
    MtpObjectFormat format = mRequest.getParameter(2);
    std::cout << "GetObjectPropDesc " << MtpDebug::getObjectPropCodeName(propCode)
            << " " << MtpDebug::getFormatCodeName(format) << std::endl;
...
std::cout << "GetDevicePropDesc " << MtpDebug::getDevicePropCodeName(propCode) << std::endl;
...
MtpResponseCode MtpServer::doSendPartialObject() {
    ...
    ObjectEdit* edit = getEditObject(handle);
    if (!edit) {
std::cout << "object not open for edit in doSendPartialObject" << std::endl;
        return MTP_RESPONSE_GENERAL_ERROR;
    }

    // can't start writing past the end of the file
    if (offset > edit->mSize) {
	    std::cout << "writing past end of object, offset: " << offset
                << " edit->mSize: " << edit->mSize << std::endl;
        return MTP_RESPONSE_GENERAL_ERROR;
    }

    const char* filePath = edit->mPath.c_str();
    std::cout << "receiving partial " << filePath
...
MtpResponseCode MtpServer::doTruncateObject() {
    ...
    ObjectEdit* edit = getEditObject(handle);
    if (!edit) {
std::cout << "object not open for edit in doTruncateObject" << std::endl;
        return MTP_RESPONSE_GENERAL_ERROR;
    }
...
std::cout << "object already open for edit in doBeginEditObject" << std::endl;
        return MTP_RESPONSE_GENERAL_ERROR;
    }
    ...
    if (fd < 0) {
std::cout << "open failed for " << path.c_str() << " in doBeginEditObject" << std::endl;
        return MTP_RESPONSE_GENERAL_ERROR;
    }
...
MtpResponseCode MtpServer::doEndEditObject() {
    ...
    ObjectEdit* edit = getEditObject(handle);
    if (!edit) {
std::cout << "object not open for edit in doEndEditObject" << std::endl;
        return MTP_RESPONSE_GENERAL_ERROR;
    }
