...
int main(int argc, char** argv)
{
    ...
    std::cout << "MTP server starting..." << std::endl;

    if (fd < 0)
	    std::cout << "Error opening /dev/mtp_usb, aborting now..." << std::endl;
 
    android::MtpServer *server = new android::MtpServer(
            fd, 
            new android::UcloudMtpDatabase("/media/internal"),
            false, 
            userdata->pw_gid, 
            FileSystemConfig::file_perm, 
            FileSystemConfig::directory_perm);

    strcpy(product_name, "arthurmtp");
    internal_storage = new android::MtpStorage(
        MTP_STORAGE_FIXED_RAM, 
        "/media/internal",
        product_name,
        1024ULL * 1024 * 100,  /* 100 MB reserved space, to avoid filling the disk */
        false,
        1024ULL * 1024ULL * 1024ULL * 2  /* 2GB arbitrary max file size */);
